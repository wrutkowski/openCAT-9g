...
var bodyY = 40;
var bodyZ = 60;
var femurLength = 48.1;
var tibiaLength = 51;

var L = (femurLength + tibiaLength) * 0.95; // max leg length
...
function draw() {
	...
	if (showPlane) {
		...
		for (var i = -planeSize / gridSize / 2 + 1; i < planeSize / gridSize / 2; i++) {
			push();
			translate(0, i * gridSize, -0.99 * L);
			fill(80);
			box(planeSize, 1, 1);
			pop();
			push();
			translate(i * gridSize, 0, -0.99 * L);
			fill(80);
			box(1, planeSize, 1);
			pop();
		}
	...
	push();
	translate(-bodyX/2 + 22, -bodyY/2 - 10, -20);
	...
	push();
	translate(bodyX/2 - 45, -bodyY/2 - 10, -20);
